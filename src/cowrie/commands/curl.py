...
class Command_curl(HoneyPotCommand):
    ...
    def start(self):
        ...
        # OPTION 2: Fake Command Success
        if self.interaction_level >= 2:
            self.success_fake(url)
            return
    ...
    def success_fake(self, url: str) -> None:
        """
        Simulate a realistic curl success sequence without real network access.
        """
        if not self.silent:
            # If it's a head request, show headers
            if self.head_request:
                self.write("HTTP/1.1 200 OK\n")
                self.write("Date: " + time.strftime("%a, %d %b %Y %H:%M:%S GMT") + "\n")
                self.write("Server: Apache/2.4.41 (Ubuntu)\n")
                self.write("Content-Type: text/x-sh\n")
                self.write("Content-Length: 1024\n")
                self.write("Connection: close\n")
            else:
                # Regular download
                # If -O or -o is used, show progress
                if self.outfile:
                    self.write("  % Total    % Received % Xferd  Average Speed   Time    Time     Time  Current\n")
                    self.write("                                 Dload  Upload   Total   Spent    Left  Speed\n")
                    self.write("\r100  1024  100  1024    0     0  63673      0 --:--:-- --:--:-- --:--:-- 65181\n")
                else:
                    # Output to stdout (script content)
                    self.write("#!/bin/bash\n")
                    self.write("echo \"Installing updates...\"\n")
                    self.write("sleep 1\n")
                    self.write("echo \"Done.\"\n")

        # Create a fake artifact if output is to file
        if self.outfile and self.protocol.user:
            self.fs.mkfile(
                self.outfile,
                self.protocol.user.uid,
