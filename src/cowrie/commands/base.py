...
class Command_ps(HoneyPotCommand):
    def call(self) -> None:
        ...
        if self.protocol.user.server.process:
            ...
            # Progressive Deception: Inject fake services at Level 2+
            if self.interaction_level >= 2:
                for proc_name in ["/usr/sbin/mysqld", "/usr/sbin/apache2", "/usr/sbin/crond"]:
                    new_pid = random.randint(1000, 3000)
                    output = (
                        "%s".ljust(15 - len("root")) % "root",
                        "%s".ljust(8 - len(str(new_pid))) % str(new_pid),
                        "%s".ljust(13 - len("0.0")) % "0.0",
                        "%s".ljust(13 - len("0.3")) % "0.3",
                        "%s".ljust(12 - len("125416")) % "125416",
                        "%s".ljust(12 - len("45024")) % "45024",
                        "%s".ljust(10 - len("?")) % "?",
                        "%s".ljust(8 - len("Ssl")) % "Ssl",
                        "%s".ljust(8 - len("Jul22")) % "Jul22",
                        "%s".ljust(8 - len("0:15")) % "0:15",
                        "%s".ljust(30 - len(proc_name)) % proc_name
                    )
                    output_array.append(output)
